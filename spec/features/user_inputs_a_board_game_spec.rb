...
feature "Input a Board Game", %q(
  ...
  [ x ] I must enter a name, publisher, description and release date
  [ x ] Description must be a minimum of 50 characters
  [ x ] If all fields are complte,  I am told that my board game has been saved
  [ x ] If a field is incomplete , I am given an error message and brought
  back to the input form
  [ x ] If a board game already exists in the database, I am given an error
  message
  [ x ] I must be signed in to add a board game.
) do
  ...
  context "authenticated user" do
    ...
    scenario "user is given an error if they forget to enter the board game
    name" do
      click_on "Create Board Game"
    ...
    scenario "user enters duplicate board game" do
      board_game = BoardGame.create(name: "Monopoly",
      publisher: "Milton Bradley",
      description: "It really never ends xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
      ...
      expect(page).to have_content("This board game already exists in our
      records!")
    end
  end
